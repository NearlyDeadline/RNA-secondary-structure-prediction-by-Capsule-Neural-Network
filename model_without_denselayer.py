...
def routing(input, b_IJ, num_outputs=3, num_dims=16):
    """
    路由算法
    参数:
        input:一个形状为[batch_size, num_caps_l=14880, 1, len_vec=8, 1]的5维张量
        num_outputs:输出胶囊的数量，因为要预测的结果只有三种可能，即左括号，右括号或点，因此是3个
        num_dims:输出向量的维度
    返回:
        一个形状为[batch_size, num_caps_l_plus_1, vec_len=16, 1]的4维张量
    ...
    W = tf.get_variable('Weight',
                        shape=[1, input_shape[1], num_dims * num_outputs, 8, 1],
                        dtype=tf.float32,
...
def interface(inputs, Y, batch_size, vec_len, temp_batch_size):
    ...
    with tf.variable_scope('Conv1_layer'):
        # 第一层为卷积层，使用大小为[3,3,64]的卷积核进行卷积操作
        # 维度变化为[batch_size,19,128,1] --> [batch_size,17,126,64]
        conv1 = tf.contrib.layers.conv2d(inputs,
                                         num_outputs=64,
                                         kernel_size=3,
                                         stride=1,
                                         padding='VALID')
        assert (conv1.shape == [temp_batch_size, 17, 126, 64])

    with tf.variable_scope('PrimaryCaps_layer'):
        # 第二层为主胶囊层，使用16个胶囊，向量长度为8，不使用动态路由算法
        # 维度变化为[batch_size,15,124,16] --> [batch_size,59520,8,1]
        num_outputs2 = 16 # 本层共有8个胶囊用以输出
        vec_len2 = 8  # 本层向量长度为2
        ...
        # 改变维度，将每一个矩阵拉伸为29760个2维向量
        # (29760=15*124*16)
        caps1 = tf.reshape(caps1,
                           shape=[temp_batch_size, -1, vec_len2, 1])
        assert (caps1.shape == [temp_batch_size, 29760, 8, 1])

    with tf.variable_scope('DigitCaps_layer'):
        # 第三层，数字胶囊层，可理解为向量版的全连接层，本层共有3个胶囊
        # 使用动态路由算法进行迭代训练
        # 维度变化为[batch_size,29760,8,1] --> [batch_size,3,4,1]

        vec_len3 = 16  # 本层向量长度为16
        digitcaps = tf.reshape(caps1,
                               shape=(temp_batch_size, -1, 1, caps1.shape[-2].value, 1))
        # [batch_size,29760,16,1] --> [batch_size,29760,1,16,1]

        with tf.variable_scope('routing'):
            # b_IJ的维度为[batch_size, num_caps_l, num_caps_l_plus_1, 1, 1]

            b_IJ = tf.constant(
                np.zeros([temp_batch_size, digitcaps.shape[1].value, num_outputs, 1, 1], dtype=np.float32))
